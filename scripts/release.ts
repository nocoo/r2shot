#!/usr/bin/env bun
/**
 * Automated release script for r2shot Chrome Extension.
 *
 ...
 *
 * Modeled after pew/scripts/release.ts, adapted for single-package use.
 */
...
const MANIFEST_JSON = pathResolve(PROJECT_ROOT, "public/manifest.json");
...
function updateJsonVersion(
  ...
  if (!content.includes(pattern)) {
    console.error(`  ✗ ${label} — pattern not found: ${pattern}`);
    return false;
  }

  writeFileSync(filePath, content.replace(pattern, replacement), "utf-8");
  console.log(`  ✓ ${label}`);
  return true;
}
...
function updateChangelog(newSection: string): void {
  ...
  const marker = "## ";
  const idx = content.indexOf(marker, content.indexOf("\n"));
...
async function main(): Promise<void> {
  ...
  if (isDryRun) {
    console.log(
      `   [dry-run] Would update ${VERSION_FILES.length} files: ${currentVersion} → ${newVersion}`,
    );
    ...
    if (!ok) {
      console.error("\n❌ Failed to update version files. Aborting.");
  ...
  const filesToStage = [...VERSION_FILES, "bun.lock", "CHANGELOG.md"];
